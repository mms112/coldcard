...
def address_explorer_check(goto_home, pick_menu_item, need_keypress, cap_menu,
    ...
    def doit(way, addr_fmt, wallet, cc_minsc_name, export_check=True):
        ...
        if export_check:
            ...
            assert cc_external.split("#")[0].replace("sc:pk_k", "s:pk").replace("c:pk_k", "pk") == external_desc.split("#")[0].replace("'", "h")
            assert cc_internal.split("#")[0].replace("sc:pk_k", "s:pk").replace("c:pk_k", "pk") == internal_desc.split("#")[0].replace("'", "h")
